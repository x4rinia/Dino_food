...
void main() {
  ...
  group('Shopping List Free-Text and Food Linking Tests', () {
    testWidgets(
      'Recognizes existing food and links food_id without prompting',
      ...
      await tester.tap(find.text('Hinzufügen'));
        ...
        final milch = foodProvider.foods.firstWhere(
          (f) => f.name.toLowerCase() == 'milch',
        );
        final item = shoppingProvider.allItems.firstWhere(
          (i) => i.displayName.toLowerCase() == 'milch',
        );
    ...
    testWidgets(
      'Prompts for unknown food and option "Nur Einkaufsliste" creates free-text item without foods entry',
      (WidgetTester tester) async {
        final foodProvider = FoodProvider();
        await foodProvider.loadFoods();
        final shoppingProvider = ShoppingProvider();
    ...
    testWidgets(
      'Prompts for unknown food and option "Lebensmittel hinzufügen" creates foods entry and links food_id',
      (WidgetTester tester) async {
        final foodProvider = FoodProvider();
        await foodProvider.loadFoods();
        final shoppingProvider = ShoppingProvider();
        ...
        final newFood = foodProvider.foods.firstWhere(
          (f) => f.name == 'Bio-Avocado',
        );

        // Shopping item is linked to this food_id
        final item = shoppingProvider.allItems.firstWhere(
          (i) => i.displayName == 'Bio-Avocado',
        );
  ...
  group('Stock Removal vs Food Deletion Tests', () {
    testWidgets(
      'Removing from stock removes household_stock but keeps food in foods',
      (WidgetTester tester) async {
        final foodProvider = FoodProvider();
        await foodProvider.loadFoods();
    ...
    testWidgets('Deleting food on FoodsScreen deletes from foods table', (
      WidgetTester tester,
    ) async {
      final foodProvider = FoodProvider();
      await foodProvider.loadFoods();
      ...
      expect(
        find.text(
          '„Einmaliger Artikel“ wirklich vollständig aus der Lebensmittel-Liste löschen?',
        ),
        findsOneWidget,
      );
